...
export function buildInviteEmail(
  ...
  const subject = `You've been invited to join ${orgName} on MosRev`;
  const text = `Hi there,

${invitedByName} has invited you to join ${orgName} on MosRev — an Islamic finance workflow approval platform.
...
<body style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px; color: #27272a;">
  ...
  <p style="font-size: 14px; line-height: 1.6; margin-bottom: 16px;">
    <strong>${escapeHtml(invitedByName)}</strong> has invited you to join <strong>${escapeHtml(orgName)}</strong> on MosRev — an Islamic finance workflow approval platform.
  </p>
